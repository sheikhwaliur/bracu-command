...
export default function PageLayout({ children, title, subtitle, eyebrow }: {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--paper)', color: 'var(--ink)', fontFamily: 'DM Sans, sans-serif' }}>
      ...
      {/* Nav */}
      <nav style={{ position: 'fixed', top: 0, left: 0, right: 0, height: '52px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 40px', background: 'rgba(255,251,248,0.95)', backdropFilter: 'blur(16px)', borderBottom: '1px solid var(--border)', zIndex: 500 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '10px', letterSpacing: '2px', textTransform: 'uppercase', color: 'var(--faded)', fontFamily: 'DM Mono, monospace' }}>
            <span style={{ width: '6px', height: '6px', borderRadius: '50%', background: 'var(--red)', display: 'inline-block' }} />Online
          </div>
          <div style={{ fontSize: '10px', color: 'var(--faded)', border: '1px solid var(--border)', padding: '5px 12px', letterSpacing: '1px', fontFamily: 'DM Mono, monospace', borderRadius: '3px' }}>
            ID: {studentId || '—'}
          </div>
          <button
            onClick={() => router.push('/dashboard')}
            style={{ fontSize: '10px', letterSpacing: '1.5px', textTransform: 'uppercase', background: 'none', border: 'none', color: 'var(--faded)', cursor: 'pointer', fontFamily: 'DM Sans, sans-serif' }}>
            ← Dashboard
          </button>
          <button
            onClick={async () => {
      ...
      {/* Strip */}
      <div style={{ overflow: 'hidden', borderBottom: '1px solid var(--border)', padding: '10px 0', background: 'rgba(35,56,107,0.02)', marginTop: '52px' }}>
        <div style={{ display: 'flex', gap: '52px', animation: 'scroll 26s linear infinite', width: 'max-content' }}>
          {[...Array(2)].flatMap(() => STRIP_ITEMS.map((t, i) => (
            <span key={t + i} style={{ fontSize: '9px', letterSpacing: '3px', textTransform: 'uppercase', color: 'var(--faded)', whiteSpace: 'nowrap', display: 'flex', alignItems: 'center', gap: '12px', fontFamily: 'DM Mono, monospace' }}>
              {t}<span style={{ color: 'rgba(35,56,107,0.3)' }}>//</span>
            </span>
          )))}
        </div>
      </div>
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '48px 40px 32px', borderBottom: '1px solid var(--border)' }}>
        {eyebrow && (
          <div style={{ fontSize: '9px', letterSpacing: '3px', textTransform: 'uppercase', color: 'var(--red)', marginBottom: '10px', display: 'flex', alignItems: 'center', gap: '10px', fontFamily: 'DM Mono, monospace' }}>
      ...
      <footer style={{ borderTop: '1px solid var(--border)', padding: '22px 40px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', maxWidth: '1200px', margin: '0 auto' }}>
        ...
        <div style={{ fontSize: '9px', color: 'var(--faded)', letterSpacing: '1px', textAlign: 'right', lineHeight: 1.8, fontFamily: 'DM Sans, sans-serif' }}>
      ...
      <button
        ...
        style={{ position: 'fixed', bottom: '32px', right: '32px', background: 'var(--red)', color: '#fffbf8', border: 'none', width: '40px', height: '40px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '16px', cursor: 'pointer', zIndex: 400, transition: 'all .2s', opacity: showTop ? 1 : 0, pointerEvents: showTop ? 'all' : 'none', borderRadius: '3px' }}>
        ↑
      </button>
