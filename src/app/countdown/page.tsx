...
function CountdownDisplay({ exam, large = false }: { exam: Exam; large?: boolean }) {
  ...
  if (time.past) {
    return (
      <div style={{ fontFamily: 'Bebas Neue,sans-serif', fontSize: large ? '28px' : '20px', color: 'var(--faded)', letterSpacing: '3px' }}>
        COMPLETED
      </div>
    )
  }
  ...
  const numSize = large ? 'clamp(36px,8vw,56px)' : 'clamp(28px,6vw,40px)'
  const lblSize = large ? '10px' : '9px'
  const colonSize = large ? 'clamp(28px,6vw,44px)' : 'clamp(22px,5vw,32px)'

  const units = [
    ...(time.days > 0 ? [{ v: time.days, l: 'DAYS' }] : []),
    { v: time.hours, l: 'HRS' },
    { v: time.mins, l: 'MIN' },
    { v: time.secs, l: 'SEC' },
  ]
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: '4px', justifyContent: 'center' }}>
      {units.map((u, i) => (
        <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
          <div style={{ textAlign: 'center', minWidth: large ? '60px' : '48px' }}>
            <div style={{
            ...
            <div style={{ fontSize: lblSize, color: 'var(--faded)', letterSpacing: '2px', marginTop: '4px' }}>
              {u.l}
            </div>
          </div>
          {i < units.length - 1 && (
...
export default function CountdownPage() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('bracu_exams')
    if (saved) setExams(JSON.parse(saved))
  }, [])

  // Save to localStorage
  useEffect(() => {
    localStorage.setItem('bracu_exams', JSON.stringify(exams))
  }, [exams])
  ...
  const nextExam = [...upcoming].sort((a, b) =>
  ...
  const lbl: React.CSSProperties = {
    ...
    color: 'var(--faded)', display: 'block', marginBottom: '6px', fontWeight: 700
  }
  ...
  return (
    ...
    >
      {/* Next exam hero */}
      {nextExam && (
        <div style={{ background: 'var(--ink2)', border: '1px solid var(--border)', marginBottom: '24px', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ padding: '20px 24px 16px' }}>
            <div style={{ fontSize: '9px', letterSpacing: '3px', textTransform: 'uppercase', color: 'var(--red)', marginBottom: '10px', fontWeight: 700 }}>// NEXT EXAM</div>
            <div style={{ fontFamily: 'Bebas Neue,sans-serif', fontSize: 'clamp(28px,6vw,42px)', color: 'var(--paper)', letterSpacing: '2px', lineHeight: 1, marginBottom: '6px' }}>
              {nextExam.course}
            </div>
            <div style={{ fontSize: '11px', color: 'var(--faded)', letterSpacing: '.5px' }}>
              {TYPE_LABEL[nextExam.type]} · {new Date(nextExam.date).toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' })} · {nextExam.time}
              {nextExam.room && ` · ${nextExam.room}`}
            </div>
          </div>

          {/* Countdown box */}
          <div style={{ background: 'var(--ink)', borderTop: '1px solid var(--border)', padding: '20px 24px' }}>
            <CountdownDisplay exam={nextExam} large={true} />
          </div>
        </div>
      )}
        ...
        <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap', marginBottom: '10px' }}>
          {['all', 'midterm', 'final', 'quiz', 'lab', 'assignment'].map(f => (
            <button key={f} onClick={() => setFilter(f)}
              style={{ padding: '7px 12px', background: filter === f ? 'var(--red)' : 'transparent', color: filter === f ? 'var(--paper)' : 'var(--faded)', border: `1px solid ${filter === f ? 'var(--red)' : 'var(--border)'}`, fontSize: '9px', letterSpacing: '1.5px', textTransform: 'uppercase', fontFamily: 'IBM Plex Mono,monospace', cursor: 'crosshair', transition: 'all .15s', whiteSpace: 'nowrap' }}>
              {f === 'all' ? 'All' : TYPE_LABEL[f]}
            </button>
          ))}
        </div>
        <button onClick={() => setShowAdd(p => !p)}
          style={{ background: 'var(--red)', color: 'var(--paper)', border: 'none', padding: '10px 20px', fontSize: '10px', letterSpacing: '2px', textTransform: 'uppercase', fontFamily: 'IBM Plex Mono,monospace', cursor: 'crosshair', width: '100%' }}>
          + Add Exam / Deadline
        </button>
      </div>
      ...
      {showAdd && (
        <div style={{ background: 'var(--ink2)', border: '1px solid var(--border)', padding: '20px', marginBottom: '16px', position: 'relative' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '10px' }}>
            <div><label style={lbl}>Course Code</label><input style={inp} placeholder="CSE471" value={form.course} onChange={e => setForm(p => ({ ...p, course: e.target.value.toUpperCase() }))} /></div>
            <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '10px' }}>
            ...
            <div><label style={lbl}>Time</label><input style={inp} type="time" value={form.time} onChange={e => setForm(p => ({ ...p, time: e.target.value }))} /></div>
          </div>

          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '16px' }}>
            <div><label style={lbl}>Room</label><input style={inp} placeholder="UB40201" value={form.room} onChange={e => setForm(p => ({ ...p, room: e.target.value }))} /></div>
            <div><label style={lbl}>Notes</label><input style={inp} placeholder="Topics to cover..." value={form.notes} onChange={e => setForm(p => ({ ...p, notes: e.target.value }))} /></div>
          </div>

          <button onClick={addExam}
            style={{ width: '100%', background: 'var(--paper)', color: 'var(--ink)', border: 'none', padding: '12px', fontFamily: 'IBM Plex Mono,monospace', fontSize: '10px', letterSpacing: '3px', textTransform: 'uppercase', fontWeight: 700, cursor: 'crosshair' }}>
            Add →
          </button>
        </div>
      )}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
        ...
        {sorted.map(e => {
          ...
          return (
            <div key={e.id}
              style={{ background: 'var(--ink)', border: `1px solid ${tl.past ? 'rgba(242,237,228,0.04)' : 'var(--border)'}`, opacity: tl.past ? 0.4 : 1, transition: 'background .15s' }}
              onMouseEnter={e2 => (e2.currentTarget.style.background = 'var(--ink2)')}
              onMouseLeave={e2 => (e2.currentTarget.style.background = 'var(--ink)')}>

              {/* Top row — type + course + remove */}
              <div style={{ padding: '14px 16px 10px', display: 'flex', alignItems: 'center', gap: '10px' }}>
                <div style={{ fontSize: '8px', letterSpacing: '2px', textTransform: 'uppercase', color: TYPE_COLOR[e.type], border: `1px solid ${TYPE_COLOR[e.type]}`, padding: '3px 8px', flexShrink: 0, opacity: 0.85 }}>
                  {e.type}
                </div>
                ...
                <button onClick={() => removeExam(e.id)}
                  style={{ marginLeft: 'auto', background: 'none', border: '1px solid var(--border)', color: 'var(--faded)', width: '28px', height: '28px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'crosshair', fontSize: '12px', flexShrink: 0, transition: 'all .15s' }}
              ...
              <div style={{ padding: '0 16px 12px' }}>
                ...
                {e.notes && <div style={{ fontSize: '10px', color: 'var(--dim)', marginTop: '4px', fontStyle: 'italic' }}>{e.notes}</div>}
              </div>

              {/* Countdown — full width */}
              <div style={{ background: 'rgba(242,237,228,0.02)', borderTop: '1px solid var(--border)', padding: '16px' }}>
