...
export default function LoginPage() {
  ...
  const handleSignup = useCallback(async () => {
    ...
    setMsg('Account created! Check your BRACU G Suite inbox and click the confirmation link.')
  ...
  const inpStyle = (name: string): React.CSSProperties => ({
    ...
    borderBottom: `1px solid ${focused === name ? 'var(--red)' : 'rgba(232,244,255,0.15)'}`,
  ...
  const subBtn: React.CSSProperties = {
    ...
    border: 'none', padding: '16px', fontFamily: 'IBM Plex Mono, monospace',
  ...
  const secBtn: React.CSSProperties = {
    ...
    marginTop: '8px', transition: 'all .2s', cursor: 'crosshair', opacity: loading ? 0.6 : 1,
  }
  ...
  const tabBtn = (m: string): React.CSSProperties => ({
    ...
    border: `1px solid ${mode === m ? 'var(--red)' : 'rgba(232,244,255,0.09)'}`,
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; cursor: crosshair; }
        ...
        input::placeholder { color: rgba(74,122,155,0.5); font-size: 13px; letter-spacing: 1px; }
        ...
        .bg-word { position: fixed; font-family: 'Bebas Neue', sans-serif; color: rgba(0,180,255,0.025); letter-spacing: 12px; pointer-events: none; user-select: none; z-index: 0; line-height: 1; }
        ...
        .login-nav { position: fixed; top: 0; left: 0; right: 0; height: 52px; display: flex; align-items: center; justify-content: space-between; padding: 0 40px; z-index: 100; background: rgba(10,14,26,0.95); backdrop-filter: blur(12px); border-bottom: 1px solid var(--border); }
        ...
        .login-foot { position: fixed; bottom: 0; left: 0; right: 0; padding: 0 40px; height: 38px; display: flex; align-items: center; justify-content: space-between; font-size: 9px; letter-spacing: 1.5px; text-transform: uppercase; color: var(--dim); border-top: 1px solid var(--border); background: var(--ink); }
        ...
        <div className="login-card">
          ...
          <div className="card-left">
            <div className="card-left-inner">
              <div style={{ display: 'inline-block', fontSize: '8px', letterSpacing: '3px', textTransform: 'uppercase', color: 'var(--red)', border: '1px solid rgba(0,180,255,0.3)', padding: '3px 9px', marginBottom: '24px', fontWeight: 700 }}>
          ...
          <div className="card-right">
            ...
            {mode === 'login' && (
              <>
                ...
                <div style={{ marginBottom: '20px' }}>
                  ...
                  <input type="text" inputMode="numeric" maxLength={8} placeholder="21301234" value={id} onChange={e => setId(e.target.value)} onFocus={() => setFocused('id')} onBlur={() => setFocused(null)} onKeyDown={e => e.key === 'Enter' && handleLogin()} style={inpStyle('id')} autoComplete="username" />
                  <div style={{ fontSize: '9px', color: 'var(--dim)', marginTop: '5px' }}>Your 8-digit BRACU student ID</div>
                </div>
                <div style={{ marginBottom: '20px' }}>
                  <label style={lbl}>Password</label>
                  <input type="password" placeholder="••••••••" value={pw} onChange={e => setPw(e.target.value)} onFocus={() => setFocused('pw')} onBlur={() => setFocused(null)} onKeyDown={e => e.key === 'Enter' && handleLogin()} style={inpStyle('pw')} autoComplete="current-password" />
                </div>
                <button onClick={handleLogin} disabled={loading} style={subBtn} onMouseEnter={() => setBtnHover(true)} onMouseLeave={() => setBtnHover(false)}>
            ...
            {mode === 'signup' && (
              <>
                ...
                <div style={{ marginBottom: '16px' }}>
                  <label style={lbl}>Student ID</label>
                  <input type="text" inputMode="numeric" maxLength={8} placeholder="21301234" value={id} onChange={e => setId(e.target.value)} onFocus={() => setFocused('sid')} onBlur={() => setFocused(null)} style={inpStyle('sid')} autoComplete="username" />
                ...
                <div style={{ marginBottom: '16px' }}>
                  ...
                  <input type="email" placeholder="username@g.bracu.ac.bd" value={gsuiteEmail} onChange={e => setGsuiteEmail(e.target.value)} onFocus={() => setFocused('gsuite')} onBlur={() => setFocused(null)} style={inpStyle('gsuite')} autoComplete="email" />
                ...
                <div style={{ marginBottom: '16px' }}>
                  ...
                  <input type="email" placeholder="your.personal@gmail.com" value={recoveryEmail} onChange={e => setRecoveryEmail(e.target.value)} onFocus={() => setFocused('recovery')} onBlur={() => setFocused(null)} style={inpStyle('recovery')} autoComplete="email" />
                  <div style={{ fontSize: '9px', color: 'var(--dim)', marginTop: '4px' }}>Used to reset password if you lose G Suite access</div>
                </div>
                <div style={{ marginBottom: '16px' }}>
                  <label style={lbl}>Password</label>
                  <input type="password" placeholder="Min 8 characters" value={pw} onChange={e => setPw(e.target.value)} onFocus={() => setFocused('spw')} onBlur={() => setFocused(null)} style={inpStyle('spw')} autoComplete="new-password" />
                </div>
                <div style={{ marginBottom: '20px' }}>
                  <label style={lbl}>Confirm Password</label>
                  <input type="password" placeholder="Repeat password" value={pw2} onChange={e => setPw2(e.target.value)} onFocus={() => setFocused('spw2')} onBlur={() => setFocused(null)} style={inpStyle('spw2')} autoComplete="new-password" />
                </div>
                <button onClick={handleSignup} disabled={loading} style={subBtn} onMouseEnter={() => setBtnHover(true)} onMouseLeave={() => setBtnHover(false)}>
              ...
              <>
                ...
                {!resetOtpSent ? (
                  <>
                    <div style={{ marginBottom: '20px' }}>
                      <label style={lbl}>Student ID</label>
                      <input type="text" inputMode="numeric" maxLength={8} placeholder="21301234" value={id} onChange={e => setId(e.target.value)} onFocus={() => setFocused('fid')} onBlur={() => setFocused(null)} style={inpStyle('fid')} autoComplete="username" />
                      <div style={{ fontSize: '9px', color: 'var(--dim)', marginTop: '4px' }}>OTP will be sent to your recovery email</div>
                    </div>
                    <button onClick={sendResetOtp} disabled={loading} style={subBtn} onMouseEnter={() => setBtnHover(true)} onMouseLeave={() => setBtnHover(false)}>
                  ...
                  <>
                    ...
                    <div style={{ marginBottom: '16px' }}>
                      ...
                      <input type="text" inputMode="numeric" maxLength={6} placeholder="123456" value={resetOtp} onChange={e => setResetOtp(e.target.value.replace(/\D/g, ''))} onFocus={() => setFocused('rotp')} onBlur={() => setFocused(null)} style={{ ...inpStyle('rotp'), fontSize: '24px', letterSpacing: '8px', textAlign: 'center' }} />
                    </div>
                    <div style={{ marginBottom: '20px' }}>
                      <label style={lbl}>New Password</label>
                      <input type="password" placeholder="Min 8 characters" value={newPw} onChange={e => setNewPw(e.target.value)} onFocus={() => setFocused('npw')} onBlur={() => setFocused(null)} style={inpStyle('npw')} autoComplete="new-password" />
                    </div>
                    <button onClick={verifyResetOtp} disabled={loading} style={subBtn} onMouseEnter={() => setBtnHover(true)} onMouseLeave={() => setBtnHover(false)}>
            ...
            <div style={{ marginTop: '24px', paddingTop: '18px', borderTop: '1px solid rgba(0,180,255,0.08)', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '3px 14px', fontSize: '9px', color: 'var(--dim)', lineHeight: 2 }}>
              <div><span style={{ color: 'var(--red)' }}>✓</span> G Suite verified</div>
              <div><span style={{ color: 'var(--red)' }}>✓</span> AES-256</div>
              <div><span style={{ color: 'var(--red)' }}>✓</span> OTP protected</div>
              <div><span style={{ color: 'var(--red)' }}>✓</span> Recovery email</div>
            </div>
          </div>
        </div>
